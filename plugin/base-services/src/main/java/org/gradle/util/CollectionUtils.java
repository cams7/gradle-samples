...
public abstract class CollectionUtils {
	...
	public static <T> T findFirst(Iterable<? extends T> source,
			Spec<? super T> filter) {
		for (T item : source) {
	...
	public static <T> boolean any(Iterable<? extends T> source,
			Spec<? super T> filter) {
	...
	/**
	 * Returns a sorted copy of the provided collection of things. Uses the
	 * provided comparator to sort.
	 */
	public static <T> List<T> sort(Iterable<? extends T> things,
	...
	/**
	 * Returns a sorted copy of the provided collection of things. Uses the
	 * natural ordering of the things.
	 */
	...
	public static <K, V> Map<K, V> filter(Map<K, V> map, Map<K, V> destination,
			Spec<Map.Entry<K, V>> filter) {
	...
	public static <R, I> R[] collectArray(I[] list, Class<R> newType,
	...
	public static <R, I> R[] collectArray(I[] list, R[] destination,
			Transformer<? extends R, ? super I> transformer) {
	...
	public static List<String> toStringList(Iterable<?> iterable) {
		return collect(iterable, new LinkedList<String>(),
				Transformers.asString());
	}
	...
	/**
	 * Recursively unpacks all the given things into a flat list, ensuring they
	 * are of a certain type.
	 *
	 * Nulls are not removed, they are left intact.
	 *
	 * If a non null object cannot be cast to the target type, a
	 ...
	 * @return A flattened list of the given things
	 */
	public static <T> List<T> flattenCollections(Class<T> type,
			Object... things) {
		if (things == null) {
			...
			if (thing.getClass().isArray()) {
				...
				for (Object thingThing : thingArray) {
					List<T> flattenCollections = flattenCollections(type,
							thingThing);
					list.addAll(flattenCollections);
				}
				return list;
			}
			...
			for (Object thing : things) {
				List<T> flattenCollections = flattenCollections(type, thing);
				list.addAll(flattenCollections);
			}
			return list;
		}
	...
	public static <T> List<T> intersection(
		...
		Iterator<? extends Collection<T>> iterator = availableValuesByDescriptor
				.iterator();
		if (iterator.hasNext()) {
	...
	public static <C extends Collection<String>> C stringize(
			Iterable<?> source, C destination) {
	...
	public static <E> boolean replace(List<E> list, Spec<? super E> filter,
	...
	public static <K, V> void collectMap(Map<K, V> destination,
	...
	public static <K, V> Map<K, V> collectMap(Iterable<? extends V> items,
			Transformer<? extends K, ? super V> keyGenerator) {
	...
	public static <T> boolean every(Iterable<? extends T> things,
	 ...
	 * @return t1
	 */
	public static <T> Collection<T> addAll(Collection<T> t1,
	 ...
	 * @see CollectionUtils#diffSetsBy(java.util.Set, java.util.Set,
	...
	/**
	 * Provides a “diff report” of how the two sets are similar and how they are
	 * different, comparing the entries by some aspect.
	 *
	 * The transformer is used to generate the value to use to compare the
	 * entries by. That is, the entries are not compared by equals by an
	 * attribute or characteristic.
	 *
	 * The transformer is expected to produce a unique value for each entry in a
	 * single set. Behaviour is undefined if this condition is not met.
	 *
	 * @param left
	 *            The set on the “left” side of the comparison.
	 * @param right
	 *            The set on the “right” side of the comparison.
	 * @param compareBy
	 *            Provides the value to compare entries from either side by
	 * @param <T>
	 *            The type of the entry objects
	 * @return A representation of the difference
	 */
	public static <T> SetDiff<T> diffSetsBy(Set<? extends T> left,
			Set<? extends T> right, Transformer<?, T> compareBy) {
		if (left == null) {
	...
	/**
	 * Creates a string with {@code toString()} of each object with the given
	 * separator.
	 *
	 * <pre>
	 * expect:
	 ...
	 * </pre>
	 *
	 * The {@code separator} must not be null and {@code objects} must not be
	 * null.
	 *
	 * @param separator
	...
	public static <T, I> T inject(T target, Iterable<? extends I> items,
			Action<InjectionStep<T, I>> action) {
		if (target == null) {
	...
	public static <T> Iterable<? extends T> unpack(
			final Iterable<? extends Factory<? extends T>> factories) {
		return new Iterable<T>() {
			private final Iterator<? extends Factory<? extends T>> delegate = factories
					.iterator();
	...
	public static <T> List<T> dedup(Iterable<T> source,
		...
		return ImmutableList.copyOf(Iterables.transform(deduped,
				new Function<Equivalence.Wrapper<T>, T>() {
