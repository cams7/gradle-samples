...
public class GradleVersion implements Comparable<GradleVersion> {
	...
	private static final Pattern VERSION_PATTERN = Pattern
			.compile("((\\d+)(\\.\\d+)+)(-(\\p{Alpha}+)-(\\d+[a-z]?))?(-(\\d{14}([-+]\\d{4})?))?");
	...
	static {
		...
		try {
			...
			Date buildTime = new SimpleDateFormat("yyyyMMddHHmmssZ")
					.parse(buildTimestamp);

			CURRENT = new GradleVersion(version, buildTime, buildNumber,
					commitId);
		} catch (Exception e) {
			throw new GradleException(String.format(
					"Could not load version details from resource '%s'.",
					resource), e);
		} finally {
			if (inputStream != null) {
				try {
	...
	private GradleVersion(String version, Date buildTime, String buildNumber,
			String commitId) {
		...
		if (matcher.group(8) != null) {
			try {
				if (matcher.group(9) != null) {
					snapshot = new SimpleDateFormat("yyyyMMddHHmmssZ").parse(
							matcher.group(8)).getTime();
				} else {
	...
	/**
	 * The base version of this version. For pre-release versions, this is the
	 * target version.
	 *
	...
	public int compareTo(GradleVersion gradleVersion) {
		...
		String[] otherMajorVersionParts = gradleVersion.versionPart
				.split("\\.");

		for (int i = 0; i < majorVersionParts.length
				&& i < otherMajorVersionParts.length; i++) {
	...
	static final class Stage implements Comparable<Stage> {
		...
		Stage(int stage, String number) {
			...
			try {
				...
				throw new RuntimeException("Invalid stage small number: "
						+ number, e);
			}
