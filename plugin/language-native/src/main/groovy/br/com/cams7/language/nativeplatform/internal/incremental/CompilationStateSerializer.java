...
public class CompilationStateSerializer implements Serializer<CompilationState> {
	...
	public CompilationStateSerializer() {
		...
		stateMapSerializer = new MapSerializer<File, CompilationFileState>(
				fileSerializer, new CompilationFileStateSerializer());
	}
	...
	private class CompilationFileStateSerializer implements
			Serializer<CompilationFileState> {
		private final Serializer<byte[]> hashSerializer = new HashSerializer();
		private final Serializer<Set<ResolvedInclude>> resolveIncludesSerializer = new SetSerializer<ResolvedInclude>(
				new ResolvedIncludeSerializer());
		...
		public CompilationFileState read(Decoder decoder) throws Exception {
			CompilationFileState fileState = new CompilationFileState(
					hashSerializer.read(decoder));
			fileState.setResolvedIncludes(resolveIncludesSerializer
					.read(decoder));
			fileState.setSourceIncludes(sourceIncludesSerializer.read(decoder));
			return fileState;
		}

		public void write(Encoder encoder, CompilationFileState value)
				throws Exception {
			hashSerializer.write(encoder, value.getHash());
			resolveIncludesSerializer.write(encoder,
					value.getResolvedIncludes());
	...
	private class ResolvedIncludeSerializer implements
			Serializer<ResolvedInclude> {
		...
		public void write(Encoder encoder, ResolvedInclude value)
				throws Exception {
	...
	private class SourceIncludesSerializer implements
			Serializer<SourceIncludes> {
		private final Serializer<String> stringSerializer = serializerFactory
				.getSerializerFor(String.class);
		private final ListSerializer<String> stringListSerializer = new ListSerializer<String>(
				stringSerializer);

		public SourceIncludes read(Decoder decoder) throws Exception {
			SourceIncludes sourceIncludes = new DefaultSourceIncludes();
			sourceIncludes.getQuotedIncludes().addAll(
					stringListSerializer.read(decoder));
			sourceIncludes.getSystemIncludes().addAll(
					stringListSerializer.read(decoder));
			sourceIncludes.getMacroIncludes().addAll(
					stringListSerializer.read(decoder));
			return sourceIncludes;
		}

		public void write(Encoder encoder, SourceIncludes value)
				throws Exception {
