...
/**
 * <p>
 * A {@link org.gradle.api.Plugin} which defines a basic project lifecycle.
 * </p>
 */
@Incubating
public class LifecycleBasePlugin implements Plugin<ProjectInternal> {
	...
	private static final List<String> PLACEHOLDER_TASKS = Arrays.asList(
			BUILD_TASK_NAME, CHECK_TASK_NAME, CLEAN_TASK_NAME,
			ASSEMBLE_TASK_NAME);
	...
	private void addClean(final ProjectInternal project) {
		project.getTasks().addPlaceholderAction(CLEAN_TASK_NAME, Delete.class,
				new Action<Delete>() {
					@Override
	...
	private void addAssemble(ProjectInternal project) {
		project.getTasks().addPlaceholderAction(ASSEMBLE_TASK_NAME,
				DefaultTask.class, new Action<TaskInternal>() {
					@Override
					public void execute(TaskInternal assembleTask) {
						assembleTask
								.setDescription("Assembles the outputs of this project.");
	...
	private void addCheck(final ProjectInternal project) {
		project.getTasks().addPlaceholderAction(CHECK_TASK_NAME,
				DefaultTask.class, new Action<TaskInternal>() {
					@Override
	...
	private void addBuild(final ProjectInternal project) {
		project.getTasks().addPlaceholderAction(BUILD_TASK_NAME,
				DefaultTask.class, new Action<DefaultTask>() {
					@Override
	...
	private void addDeprecationWarningsAboutCustomLifecycleTasks(
			ProjectInternal project) {
		project.getTasks().all(new Action<Task>() {
			@Override
			public void execute(Task task) {
				if (PLACEHOLDER_TASKS.contains(task.getName())) {
					DeprecationLogger.nagUserOfDeprecated(String.format(
							CUSTOM_LIFECYCLE_TASK_DEPRECATION_MSG,
							task.getName()));
				}
			}
		});
