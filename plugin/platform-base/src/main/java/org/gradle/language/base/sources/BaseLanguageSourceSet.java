...
/**
 * Base class for custom language sourceset implementations. A custom
 * implementation of {@link org.gradle.language.base.LanguageSourceSet} must
 * extend this type.
 */
public abstract class BaseLanguageSourceSet extends
		AbstractBuildableModelElement implements LanguageSourceSetInternal {
	private String name;
	...
	// TODO:DAZ This is only here as a convenience for subclasses to create
	// additional SourceDirectorySets
	...
	public String getDisplayName() {
		return String
				.format("%s '%s:%s'", getTypeName(), parentName, getName());
	}
	...
	public static <T extends BaseLanguageSourceSet> T create(Class<T> type,
			String name, String parentName, FileResolver fileResolver,
		...
		nextSourceSetInfo.set(new SourceSetInfo(name, parentName, type
				.getSimpleName(), fileResolver));
		try {
			try {
				return instantiator.newInstance(type);
			} catch (ObjectInstantiationException e) {
				throw new ModelInstantiationException(String.format(
						"Could not create LanguageSourceSet of type %s",
	...
	private static class SourceSetInfo {
		...
		private SourceSetInfo(String name, String parentName, String typeName,
