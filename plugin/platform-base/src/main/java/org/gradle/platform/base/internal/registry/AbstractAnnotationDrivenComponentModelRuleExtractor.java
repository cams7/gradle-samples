	...
	protected void assertIsVoidMethod(MethodRuleDefinition<?, ?> ruleDefinition) {
		if (!ModelType.of(Void.TYPE).equals(ruleDefinition.getReturnType())) {
			throw new InvalidModelException(
					String.format("Method %s must not have a return value.",
							getDescription()));
		}
	}

	protected <V> void visitCollectionBuilderSubject(
			RuleMethodDataCollector dataCollector,
			MethodRuleDefinition<?, ?> ruleDefinition, Class<V> typeParameter) {
		if (ruleDefinition.getReferences().size() == 0) {
			throw new InvalidModelException(String.format(
					"Method %s must have a parameter of type '%s'.",
					getDescription(), CollectionBuilder.class.getName()));
		}
		...
		if (!ModelType.of(CollectionBuilder.class).isAssignableFrom(builder)) {
			throw new InvalidModelException(String.format(
					"Method %s first parameter must be of type '%s'.",
					getDescription(), CollectionBuilder.class.getName()));
		}
		if (builder.getTypeVariables().size() != 1) {
			throw new InvalidModelException(
					String.format(
							"Parameter of type '%s' must declare a type parameter extending '%s'.",
							CollectionBuilder.class.getSimpleName(),
		...
		if (subType.isWildcard()) {
			throw new InvalidModelException(
					String.format(
							...
							typeParameter.getName(), subType.toString()));
		}
		dataCollector.parameterTypes.put(typeParameter,
				subType.getConcreteClass());
	}
		...
		public <S> void put(Class<S> baseClass, Class<? extends S> concreteClass) {
			if (!baseClass.isAssignableFrom(concreteClass)) {
				throw new InvalidParameterException(String.format(
						"Class %s must be assignable from Class %s",
						baseClass.getName(), concreteClass.getName()));
			}
			parameterTypes.put(baseClass, concreteClass);
		}
	}

	protected <S> void visitDependency(RuleMethodDataCollector dataCollector,
			MethodRuleDefinition<?, ?> ruleDefinition,
			ModelType<S> expectedDependency) {
		// TODO:DAZ Use ModelType.toString instead of getSimpleName()
		List<ModelReference<?>> references = ruleDefinition.getReferences();
		ModelType<? extends S> dependency = null;
		for (ModelReference<?> reference : references) {
			ModelType<? extends S> newDependency = expectedDependency
					.asSubclass(reference.getType());
			if (newDependency != null) {
				if (dependency != null) {
					throw new InvalidModelException(
							String.format(
									"Method %s must have one parameter extending %s. Found multiple parameter extending %s.",
									getDescription(),
									expectedDependency.getConcreteClass()
											.getSimpleName(),
									expectedDependency.getConcreteClass()
											.getSimpleName()));
		...
		if (dependency == null) {
			throw new InvalidModelException(
					String.format(
							"Method %s must have one parameter extending %s. Found no parameter extending %s.",
							getDescription(), expectedDependency
									.getConcreteClass().getSimpleName(),
							expectedDependency.getConcreteClass()
									.getSimpleName()));
		}
		dataCollector.put(expectedDependency.getConcreteClass(),
				dependency.getConcreteClass());
	}
}
