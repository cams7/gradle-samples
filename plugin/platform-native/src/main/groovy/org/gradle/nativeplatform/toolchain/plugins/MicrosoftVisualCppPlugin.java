...
/**
 * A {@link Plugin} which makes the Microsoft Visual C++ compiler available to
 * compile C/C++ code.
 */
@Incubating
public class MicrosoftVisualCppPlugin implements Plugin<Project> {
	...
	static class Rules extends RuleSource {
		...
		public static void addToolChain(
			...
			final OperatingSystem operatingSystem = serviceRegistry
			...
			final VisualStudioLocator visualStudioLocator = serviceRegistry
					.get(VisualStudioLocator.class);
			final WindowsSdkLocator windowsSdkLocator = serviceRegistry
					.get(WindowsSdkLocator.class);

			toolChainRegistry.registerFactory(VisualCpp.class,
					new NamedDomainObjectFactory<VisualCpp>() {
						public VisualCpp create(String name) {
							return instantiator.newInstance(
									VisualCppToolChain.class, name,
									buildOperationProcessor, operatingSystem,
									fileResolver, execActionFactory,
									visualStudioLocator, windowsSdkLocator,
									instantiator);
						}
					});
			toolChainRegistry.registerDefaultToolChain(
					VisualCppToolChain.DEFAULT_NAME, VisualCpp.class);
		}
