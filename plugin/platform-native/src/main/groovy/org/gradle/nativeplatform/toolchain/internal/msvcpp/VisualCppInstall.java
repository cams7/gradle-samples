...
public class VisualCppInstall implements Named {
	...
	public VisualCppInstall(String name, VersionNumber version,
			Map<Architecture, List<File>> paths,
			Map<Architecture, File> binaryPaths,
			Map<Architecture, File> libraryPaths,
			Map<Architecture, File> includePaths,
			Map<Architecture, String> assemblerFilenames,
	...
	public boolean isSupportedPlatform(NativePlatformInternal targetPlatform) {
		...
		// TODO:MPUT - ARM also if the target OS is Windows RT or Windows
		// Phone/Mobile/CE
	...
	public File getCompiler(NativePlatformInternal targetPlatform) {
		return new File(
				binaryPaths.get(getPlatformArchitecture(targetPlatform)),
				COMPILER_FILENAME);
	}
	...
	private Architecture getPlatformArchitecture(
			NativePlatformInternal targetPlatform) {
