...
abstract public class NativeCompiler<T extends NativeCompileSpec> implements
	...
	public NativeCompiler(BuildOperationProcessor buildOperationProcessor,
	...
	public WorkResult execute(T spec) {
		...
		final BuildOperationQueue<CommandLineToolInvocation> buildQueue = buildOperationProcessor
		...
		for (File sourceFile : transformedSpec.getSourceFiles()) {
			CommandLineToolInvocation perFileInvocation = createPerFileInvocation(
					genericArgs, sourceFile, objectDir, spec);
	...
	protected List<String> getArguments(T spec) {
		...
		Action<List<String>> userArgTransformer = invocationContext
				.getArgAction();
		// modifies in place
	...
	protected File getOutputFileDir(File sourceFile, File objectFileDir,
		...
		File outputFile = new CompilerOutputFileNamingScheme()
				...
				.withOutputBaseFolder(objectFileDir).map(sourceFile);
		File outputDirectory = outputFile.getParentFile();
		if (!outputDirectory.exists()) {
			outputDirectory.mkdirs();
		}
		return windowsPathLimitation ? FileUtils
				.assertInWindowsPathLengthLimitation(outputFile) : outputFile;
	}
	...
	protected List<String> maybeGetPCHArgs(final T spec, File sourceFile) {
		...
		final SourceIncludes includes = spec.getSourceFileIncludes().get(
				sourceFile);
		boolean usePCH = CollectionUtils.every(spec.getPreCompiledHeaders(),
				new Spec<String>() {
					@Override
	...
	protected CommandLineToolInvocation createPerFileInvocation(
		...
		List<String> outputArgs = getOutputArgs(spec,
				getOutputFileDir(sourceFile, objectDir, objectFileSuffix));
		List<String> pchArgs = maybeGetPCHArgs(spec, sourceFile);

		return invocationContext.createInvocation(
				"compiling ".concat(sourceFile.getName()), objectDir,
